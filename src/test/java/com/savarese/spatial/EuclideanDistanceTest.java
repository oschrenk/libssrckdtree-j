...
public class EuclideanDistanceTest extends TestCase {
  ...
  public void testDistance() {
    ...
    // 1-D
    assertEquals(d.distance2(from, to), 0.0);
    assertEquals(d.distance(from, to), 0.0);

    to.setCoord(0, 2);

    assertEquals(d.distance2(from, to), 1.0);
    assertEquals(d.distance(from, to), 1.0);

    from.setCoord(0, 48);
    to.setCoord(0, 52);

    assertEquals(d.distance2(from, to), 16.0);
    assertEquals(d.distance(from, to), 4.0);

    from.setCoord(0, 4);
    to.setCoord(0, 1);

    assertEquals(d.distance2(from, to), 9.0);
    ...
    assertEquals(d.distance2(from, to), 2.0);
    ...
    assertEquals(d.distance2(from, to), 68438025.0);
    ...
    assertEquals(d.distance2(from, to), 4.0);
